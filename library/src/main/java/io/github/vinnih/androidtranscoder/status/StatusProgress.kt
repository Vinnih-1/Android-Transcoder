...
internal class StatusProgress(val stepsValue: Int, val progress: (Int) -> Unit) {
    ...
    fun calculateProgress(totalValue: Long, currentValue: Long): Int {
        updateProgressValue()

        return ceil(((currentValue.toFloat() / totalValue.toFloat()) * 100)).toInt()
    }

    fun updateEncodeProgress(totalValue: Long, currentValue: Long) {
        encodeProgress = calculateProgress(totalValue, currentValue) / stepsValue
        status = Status.ENCODING
    }

    fun updateDecodeProgress(totalValue: Long, currentValue: Long) {
